...
@section('backend-main')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">{{ __('dashboard.deliveries') }}</h4>
                    <div style="display: flex;justify-content: space-between;">
                        <a href="{{ route('deliveries.create') }}" class="btn btn-success mb-2">
                            <i class="mdi mdi-plus mr-2"></i>
                            {{ __('dashboard.add_new') }}</a>
                        <a class="btn btn-danger mb-2  delete-all text-white" onclick="return false;"
                           delete_url="/delete_deliveries/"><i class="mdi mdi-trash-can-outline mr-2"></i>
                            ...
                            <tr>
                                ...
                                <td width="100" height="100">
                                    @if(isset($del->mainImage))
                                        <img style="width: 100%;border-radius: 10px" src="{{ asset('pictures/deliveries/' . $del->mainImage->image) }}"/>
                                    @else
                                ...
                                <td>{{ $del->name }}</td>
                                <td>{{ $del->phone }}</td>
                                <td>{{ $del->orders->count() }}</td>
                                <td>{{ $del->getActive() }}</td>
                                <td>
                                    <a href="{{ route('deliveries.edit' , $del->id) }}"
                                    ...
                                    <a title="" onclick="return false;" object_id="{{ $del->id }}"
                                       delete_url="/deliveries/" class="text-danger remove-alert" href="#"><i
