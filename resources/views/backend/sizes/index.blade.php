...
@section('backend-main')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">{{ __('dashboard.sizes') }}</h4>
                    <div style="display: flex;justify-content: space-between;">
                        <a href="{{ route('sizes.create') }}" class="btn btn-success mb-2">
                            <i class="mdi mdi-plus mr-2"></i>
                            {{ __('dashboard.add_new') }}</a>
                        <a class="btn btn-danger mb-2  delete-all text-white" onclick="return false;"
                           delete_url="/delete_sizes/"><i class="mdi mdi-trash-can-outline mr-2"></i>
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                        ...
                        @foreach($sizes as $size)
                            <tr>
                                <td>{{ $loop->index + 1 }}</td>
                                <td>{{ $size->code }}</td>
                                <td>{{ $size->getActive() }}</td>
                                <td>
                                    <a href="{{ route('sizes.edit' , $size->id) }}"
                                       class="mr-3 text-primary"><i class="mdi mdi-pencil font-size-18"></i></a>
                                    <a title="" onclick="return false;" object_id="{{ $size->id }}"
                                       delete_url="/sizes/" class="text-danger remove-alert" href="#"><i
