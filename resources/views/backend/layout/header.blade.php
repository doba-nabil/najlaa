                            ...
                            @foreach (Auth::user()->unreadNotifications as $notification)
                                ...
                                @if(Auth::user()->can('contact-list') || Auth::user()->can('contact-delete'))
                                    @if(isset($notification->data['contact']))
                                        <a href="{{ route('contacts.show' ,$notification->data['contact']['id'] ) }}"
                                           class="text-reset notification-item deletenot"
                                           notification="{{ $notification->id }}" data-token="{{ csrf_token() }}">
                                            <div class="media">
                                                <div class="avatar-xs mr-3">
                                    ...
                                    @if(isset($notification->data['ordercontact']))
                                        ...
                                        ?>
                                        <a href="{{ route('ordercontacts.show' ,$notification->data['ordercontact']['id'] ) }}"
                                           class="text-reset notification-item deletenot"
                                           notification="{{ $notification->id }}" data-token="{{ csrf_token() }}">
                                            <div class="media">
                                                <div class="avatar-xs mr-3">
                                ...
                                @if(Auth::user()->can('user-list') || Auth::user()->can('user-create') || Auth::user()->can('user-edit') || Auth::user()->can('user-delete'))
                                    @if(isset($notification->data['user']) && !isset($notification->data['order']))
                                        ...
                                        ?>
                                        <a href="@if(isset($user)){{ route('users.edit' , $user->id ) }}@else / @endif"
                                           class="text-reset notification-item deletenot"
                                           notification="{{ $notification->id }}" data-token="{{ csrf_token() }}">
                                            <div class="media">
                                                <div class="avatar-xs mr-3">
            ...
            @if(Auth::user()->can('product-list') || Auth::user()->can('product-create') || Auth::user()->can('product-edit') || Auth::user()->can('product-delete'))
                ...
                <div class="dropdown d-inline-block">
                    <button type="button" class="btn header-item noti-icon waves-effect"
                        ...
                        <?php
                        $notss = \App\Models\EmptyProductNotification::where('moderator_id', Auth::user()->id)->get();
                        ?>
                        @if(count($notss) > 0)
                            <span class="noti-dot"></span>
                        @endif
                    ...
                    <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right p-0"
                            ...
                            <div class="row align-items-center">
                                <div class="col">
                                    <h6 class="m-0">
                                        ...
                                        @else
                                            Finished products from the Stock
                                        @endif
                                    </h6>
                                </div>
                            </div>
                        ...
                        <div data-simplebar style="max-height: 230px;">
                            @if(count($notss) > 0)
                                @foreach($notss as $not)
                                    <a href="{{ route('see_empty_not' , $not->product->slug) }}"
                                       class="text-reset notification-item">
                                        <div class="media">
                                            <div class="avatar-xs mr-3">
                                        <span style="background: {{ $not->color->color }}"
                                              class="avatar-title bg- rounded-circle font-size-16">
                                            <i class="ri-shopping-cart-line"></i>
                                        </span>
                                            </div>
                                            <div class="media-body">
                                                <h6 class="mt-0 mb-1">
                                                    @if(app()->getLocale() == 'ar')
                                                        منتج منتهي من المخزن
                                                    @else
                                                        Finished product from the Stock
                                                    @endif
                                                    " {{ $not->product['name_'.app()->getLocale()] ?? '' }} "
                                                    <small>( {{ $not->size->code }} )</small>
                                                </h6>
                                                <div class="font-size-12 text-muted">
                                                    <p class="mb-1">
                                                        @if(app()->getLocale() == 'ar')
                                                            قم بالضغط لزيارة المنتج
                                                        @else
                                                            Click To Visit Product
                                                        @endif
                                                    </p>
                                                    <p class="mb-0"><i
                                                                class="mdi mdi-clock-outline"></i>{{ $not->created_at->diffForHumans() }}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
